...
label(p,"{\scriptsize pegas memanjang}",point(yaw_lengthen,0.75),align=W);
path yaw_shorten = shift(0,-20)*subpath(yaw,1,4);
draw(p,yaw_shorten,arrow=None);
label(p,"{\scriptsize pegas memendek}",point(yaw_shorten,3.25)+(1,0),align=E);
...
// xlimits( -1, 3pi);
xaxis(p,Label("\makebox[0em][l]{\footnotesize waktu~$t$}",black),xmin=-0.2,xmax=2pi+0.4,gray(0.5),arrow=None);
// xaxis(p, xmin=-0.2,xmax=2pi+0.4,gray(0.5),arrow=None);
// ylimits(-1.2, 1.2);
yaxis(p,Label("\makebox[0em][r]{\footnotesize posisi~$x$}",black),ymin=-1.2,ymax=1.2,gray(0.5),arrow=None);
